...
- [RustFS](https://rustfs.com/) is fine. Basically painless to set up and use.
...
- [PostgreSQL](https://www.postgresql.org) is awesome, but for this project I should have just used [SQLite](https://sqlite.org). I used SQLite to journal the migration process from the old app to the new, which involved recording data transfer states for all of the records in the old and new app, and it was totally fine at that scale. I could have reduced the database from a container and mounted volume to a single file, and also saved myself a lot of future upgrade pain.
