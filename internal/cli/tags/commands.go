...
func getTag(app *cli.App, name string) error {
	...
	if err := cli.CheckJSONResponse(resp.StatusCode(), resp.Body, http.StatusOK, resp.JSON200); err != nil {
		return fmt.Errorf("get tag/%s: %w", name, err)
	}
	tag := *resp.JSON200
...
func createTag(app *cli.App, name string, tag types.Tag) error {
	...
	default:
		return fmt.Errorf("check whether tag/%s exists: %w", name, cli.CheckResponseStatus(getResp.StatusCode(), getResp.Body, http.StatusNotFound))
	}
...
func editTag(app *cli.App, name string) error {
	...
	if err := cli.CheckJSONResponse(resp.StatusCode(), resp.Body, http.StatusOK, resp.JSON200); err != nil {
		return fmt.Errorf("get tag/%s for editing: %w", name, err)
	}
	tag := *resp.JSON200
	...
	if err := cli.CheckJSONResponse(putResp.StatusCode(), putResp.Body, http.StatusOK, putResp.JSON200); err != nil {
		return fmt.Errorf("update tag/%s: %w", name, err)
	}
